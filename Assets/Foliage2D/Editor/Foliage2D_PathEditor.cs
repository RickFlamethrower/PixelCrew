    ...
    public class Foliage2D_PathEditor : Editor
    {
        ...
        private void UpdateHandles(Foliage2D_Path path2D, GUIStyle iconStyle)
        {
            ...
            for (int i = 0; i < path2D.handlesPosition.Count; i++)
            {
                ...
                var fmh_216_54_638500957985582736 = Quaternion.identity; global = Handles.FreeMoveHandle(pos, HandleScale(pos), Vector3.zero, Handles.CircleHandleCap);

                if (!path2D.uniformValues && path2D.foliagePathType == Foliage2D_PathType.Smooth)
                    { var fmh_219_60_638500957985612043 = Quaternion.identity; tGlobal = Handles.FreeMoveHandle(tPos, HandleScale(tPos), Vector3.zero, Handles.CircleHandleCap); }
