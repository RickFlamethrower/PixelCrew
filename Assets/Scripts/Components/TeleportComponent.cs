...
namespace PixelCrew.Components
{
    public class TeleportComponent : MonoBehaviour
    {
        ...
        private IEnumerator AnimateTeleport(GameObject target)
        {
            ...
            yield return AlphaAnimation(sprite, 0); //медлееное обесцвечивание персонажа
            target.SetActive(false);

            yield return MoveAnimation(target);  //  постепенный перенос персонажа
        ...
        private IEnumerator AlphaAnimation(SpriteRenderer sprite, float destAlpha)
        {
            ...
            while (time < _alphaTime) 
            {
                ...
                var tapAlpha = Mathf.Lerp(spriteAlpha, destAlpha, progress);
